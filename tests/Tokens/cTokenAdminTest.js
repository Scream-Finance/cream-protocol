...
describe("CTokenAdmin", () => {
  ...
  describe("getCTokenAdmin", () => {
    it("it is normal admin", async () => {
      ...
      expect(
        await call(cTokenAdmin, "getCTokenAdmin", [cToken._address])
      ).toEqual(root);
    });
  ...
  describe("_setPendingAdmin()", () => {
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(cTokenAdmin, "_setPendingAdmin", [cToken._address, others], {
          from: others,
        })
    ...
    it("should properly set pending admin", async () => {
      expect(
        await send(cTokenAdmin, "_setPendingAdmin", [cToken._address, others], {
          from: admin,
        })
      ).toSucceed();
  ...
  describe("_acceptAdmin()", () => {
    beforeEach(async () => {
      cToken = await makeCToken();
      expect(
        await send(cToken, "_setPendingAdmin", [cTokenAdmin._address])
      ).toSucceed();
    });

    it("should only be callable by admin", async () => {
      await expect(
        send(cTokenAdmin, "_acceptAdmin", [cToken._address], { from: others })
      ).rejects.toRevert("revert only the admin may call this function");
    ...
    it("should succeed and set admin and clear pending admin", async () => {
      expect(
        await send(cTokenAdmin, "_acceptAdmin", [cToken._address], {
          from: admin,
        })
      ).toSucceed();
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(
          cTokenAdmin,
          "_setComptroller",
          [cToken._address, newComptroller._address],
          { from: others }
        )
      ).rejects.toRevert("revert only the admin may call this function");
  ...
  describe("_setReserveFactor()", () => {
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(cTokenAdmin, "_setReserveFactor", [cToken._address, factor], {
          from: others,
        })
      ).rejects.toRevert("revert only the admin may call this function");
  ...
  describe("_reduceReserves()", () => {
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(cTokenAdmin, "_reduceReserves", [cToken._address, reduction], {
          from: others,
        })
      ).rejects.toRevert("revert only the admin may call this function");

      expect(
        await call(cToken.underlying, "balanceOf", [cTokenAdmin._address])
      ).toEqualNumber(0);
    });
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(
          ...
          [cToken._address, newModel._address],
          { from: others }
        )
      ).rejects.toRevert("revert only the admin may call this function");
  ...
  describe("_setCollateralCap()", () => {
    ...
    beforeEach(async () => {
      cCollateralCapErc20 = await makeCToken({
        kind: "ccollateralcap",
        admin: cTokenAdmin._address,
      });
      cToken = await makeCToken({ admin: cTokenAdmin._address });
    });
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(
          cTokenAdmin,
          "_setCollateralCap",
          [cCollateralCapErc20._address, cap],
          { from: others }
        )
      ).rejects.toRevert("revert only the admin may call this function");
    ...
    it("should fail for not CCollateralCapErc20 token", async () => {
      await expect(
        send(cTokenAdmin, "_setCollateralCap", [cToken._address, cap], {
          from: admin,
        })
    ...
    it("should succeed and set new collateral cap", async () => {
      ...
      expect(await call(cCollateralCapErc20, "collateralCap")).toEqualNumber(
        cap
      );
    });
  });
  ...
  describe("_setImplementation()", () => {
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(
          cTokenAdmin,
          "_setImplementation",
          [cToken._address, cCapableDelegate._address, true, "0x0"],
          { from: others }
        )
      ).rejects.toRevert("revert only the admin may call this function");
    });
    ...
    it("should succeed and set new implementation", async () => {
      ...
      expect(await call(cToken, "implementation")).toEqual(
        cCapableDelegate._address
      );
    });
  });
  ...
  describe("extractReserves()", () => {
    ...
    beforeEach(async () => {
      ...
      expect(
        await send(cEth, "harnessSetTotalReserves", [reserves])
      ).toSucceed();
      expect(
        await send(cToken.underlying, "harnessSetBalance", [
          cToken._address,
      ...
      await send(cTokenAdmin, "setReserveManager", [reserveManager], {
        from: admin,
      });
    });
    ...
    it("should only be callable by reserve manager", async () => {
      await expect(
        send(cTokenAdmin, "extractReserves", [cToken._address, reduction])
      ).rejects.toRevert(
        "revert only the reserve manager may call this function"
      );

      expect(
        await call(cToken.underlying, "balanceOf", [reserveManager])
      ).toEqualNumber(0);
    });
      ...
      expect(afterBalances).toEqual(
        await adjustBalances(beforeBalances, [
          [
            ...
            reserveManager,
            "eth",
            reduction.minus(await etherGasCost(receipt)),
          ],
        ])
      );
    });
  ...
  describe("seize()", () => {
    ...
    it("should only be callable by admin", async () => {
      await expect(
        send(cTokenAdmin, "seize", [erc20._address], { from: others })
      ).rejects.toRevert("revert only the admin may call this function");

      expect(
        await call(erc20, "balanceOf", [cTokenAdmin._address])
    ...
    it("should succeed and seize tokens", async () => {
      expect(
        await send(cTokenAdmin, "seize", [erc20._address], { from: admin })
      ).toSucceed();
    ...
    it("should succeed and seize non-standard tokens", async () => {
      expect(
        await send(cTokenAdmin, "seize", [nonStandardErc20._address], {
          from: admin,
        })
      ).toSucceed();

      expect(
        await call(nonStandardErc20, "balanceOf", [cTokenAdmin._address])
      ).toEqualNumber(0);
  ...
  describe("setAdmin()", () => {
    it("should only be callable by admin", async () => {
      await expect(
        send(cTokenAdmin, "setAdmin", [others], { from: others })
      ).rejects.toRevert("revert only the admin may call this function");
    ...
    it("cannot set admin to zero address", async () => {
      await expect(
        send(cTokenAdmin, "setAdmin", [address(0)], { from: admin })
    ...
    it("should succeed and set new admin", async () => {
      expect(
        await send(cTokenAdmin, "setAdmin", [others], { from: admin })
      ).toSucceed();
  ...
  describe("setReserveManager()", () => {
    it("should only be callable by admin", async () => {
      await expect(
        send(cTokenAdmin, "setReserveManager", [reserveManager], {
          from: others,
        })
      ).rejects.toRevert("revert only the admin may call this function");
