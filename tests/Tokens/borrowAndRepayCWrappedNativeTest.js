...
async function preBorrow(cToken, borrower, borrowAmount) {
  ...
  await send(cToken.underlying, "deposit", [], {
    from: root,
    value: borrowAmount,
  });
...
async function repayBorrowNative(cToken, borrower, repayAmount) {
  await send(cToken, "harnessFastForward", [1]);
  return send(cToken, "repayBorrowNative", [], {
    from: borrower,
    value: repayAmount,
  });
}
...
async function repayBorrowBehalfNative(cToken, payer, borrower, repayAmount) {
  await send(cToken, "harnessFastForward", [1]);
  return send(cToken, "repayBorrowBehalfNative", [borrower], {
    from: payer,
    value: repayAmount,
  });
}
...
describe("CWrappedNative", function () {
  ...
  describe("borrowFresh", () => {
    ...
    it("fails if comptroller tells it to", async () => {
      ...
      ).toHaveTrollReject("BORROW_COMPTROLLER_REJECTION");
    });
    ...
    it("fails if protocol has less than borrowAmount of underlying", async () => {
      expect(
        await borrowFresh(cToken, borrower, borrowAmount.plus(1))
      ).toHaveTokenFailure(
  ...
  describe("repayBorrowNative", () => {
    ...
    it("reverts if interest accrual fails", async () => {
      await send(cToken.interestRateModel, "setFailBorrowRate", [true]);
      await expect(
        repayBorrowNative(cToken, borrower, repayAmount)
      ).rejects.toRevert("revert INTEREST_RATE_MODEL_ERROR");
    });
    ...
    it("returns success from repayBorrowFresh and repays the right amount", async () => {
      ...
      expect(
        await repayBorrowNative(cToken, borrower, repayAmount)
      ).toSucceed();
    ...
    it("reverts if overpaying", async () => {
      ...
      await expect(
        repayBorrowNative(cToken, borrower, tooMuch)
      ).rejects.toRevert("revert subtraction underflow");
    });
  });
  ...
  describe("repayBorrow", () => {
    beforeEach(async () => {
      ...
      await send(
        cToken.underlying,
        "approve",
        [cToken._address, repayAmount.multipliedBy(2)],
        { from: borrower }
      );
    });
  ...
  describe("repayBorrowBehalfNative", () => {
    ...
    it("reverts if interest accrual fails", async () => {
      await send(cToken.interestRateModel, "setFailBorrowRate", [true]);
      await expect(
        repayBorrowBehalfNative(cToken, payer, borrower, repayAmount)
      ).rejects.toRevert("revert INTEREST_RATE_MODEL_ERROR");
    });
    ...
    it("reverts from within repay borrow fresh", async () => {
      ...
      await expect(
        repayBorrowBehalfNative(cToken, payer, borrower, repayAmount)
      ).rejects.toRevert("revert repay behalf native failed");
    });
    ...
    it("returns success from repayBorrowFresh and repays the right amount", async () => {
      ...
      const beforeAccountBorrowSnap = await borrowSnapshot(cToken, borrower);
      expect(
        await repayBorrowBehalfNative(cToken, payer, borrower, repayAmount)
      ).toSucceed();
      const afterAccountBorrowSnap = await borrowSnapshot(cToken, borrower);
      expect(afterAccountBorrowSnap.principal).toEqualNumber(
        beforeAccountBorrowSnap.principal.minus(repayAmount)
  ...
  describe("repayBorrowBehalf", () => {
    ...
    it("reverts from within repay borrow fresh", async () => {
      ...
      await expect(
        repayBorrowBehalf(cToken, payer, borrower, repayAmount)
      ).rejects.toRevert("revert repay behalf failed");
    });
