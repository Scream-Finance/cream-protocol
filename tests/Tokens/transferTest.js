...
describe("CToken", function () {
  ...
  describe("transfer", () => {
    ...
    it("doesn't transfer when src == dst", async () => {
      ...
      await send(cToken, "harnessSetBalance", [root, 100]);
      expect(await call(cToken, "balanceOf", [root])).toEqualNumber(100);
      expect(await send(cToken, "transfer", [root, 50])).toHaveTokenFailure(
        "BAD_INPUT",
    ...
    it("rejects transfer when not allowed and reverts if not verified", async () => {
      ...
      expect(await send(cToken, "transfer", [root, 50])).toHaveTrollReject(
    ...
    it("transfers cslp token", async () => {
      ...
      expect(await call(cToken, "balanceOf", [minter])).toEqualNumber(
        mintTokens
      );
      ...
      await send(cToken, "transfer", [accounts[0], mintTokens], {
        from: minter,
      });
      expect(await call(cToken, "balanceOf", [minter])).toEqualNumber(0);
      expect(await call(cToken, "balanceOf", [accounts[0]])).toEqualNumber(
        mintTokens
      );

      expect(await balanceOf(sushi, minter)).toEqualNumber(etherMantissa(0));
      expect(await call(cToken, "xSushiUserAccrued", [minter])).toEqualNumber(
        etherMantissa(1)
      );
    ...
    describe("transfer ccollateralcap token", () => {
      it("transfers collateral tokens", async () => {
        ...
        expect(await call(cToken, "balanceOf", [root])).toEqualNumber(100);
        expect(
          await call(cToken, "accountCollateralTokens", [root])
        ).toEqualNumber(100);
        await send(cToken, "transfer", [accounts[0], 50]);
        expect(await call(cToken, "balanceOf", [root])).toEqualNumber(50);
        expect(
        ...
        expect(
          await call(cToken, "accountCollateralTokens", [accounts[0]])
        ).toEqualNumber(50);
      });
