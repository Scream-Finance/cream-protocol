...
describe("CToken", function () {
  ...
  describe("mintFresh", () => {
    ...
    it("succeeds but reach collateracl cap", async () => {
      expect(
        await send(cToken, "_setCollateralCap", [mintTokens.minus(1)])
      ).toSucceed();
      expect(await mintFresh(cToken, minter, mintAmount)).toSucceed();
  ...
  [redeemFreshTokens, redeemFreshAmount].forEach((redeemFresh) => {
    describe(redeemFresh.name, () => {
      ...
      it("fails if comptroller tells it to", async () => {
        ...
        ).rejects.toRevert("revert comptroller rejection");
      });
      ...
      it("succeeds and not consume collateral", async () => {
        await send(cToken, "harnessSetBalance", [
          redeemer,
          redeemTokens.multipliedBy(3),
        ]);
        await send(cToken, "harnessSetCollateralBalance", [
          redeemer,
          redeemTokens,
        ]);
        await send(cToken, "harnessSetTotalSupply", [
          redeemTokens.multipliedBy(3),
        ]);
