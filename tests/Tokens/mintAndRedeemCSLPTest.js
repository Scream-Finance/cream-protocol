...
describe("CToken", function () {
  ...
  describe("mintFresh", () => {
    ...
    it("fails if comptroller tells it to", async () => {
      ...
      expect(await mintFresh(cToken, minter, mintAmount)).toHaveTrollReject(
    ...
    it("fails if not fresh", async () => {
      ...
      expect(await mintFresh(cToken, minter, mintAmount)).toHaveTokenFailure(
        "MARKET_NOT_FRESH",
  ...
  describe("mint", () => {
    ...
    it("claims sushi rewards after minting", async () => {
      ...
      expect(await balanceOf(sushi, minter)).toEqualNumber(
  ...
  [redeemFreshTokens, redeemFreshAmount].forEach((redeemFresh) => {
    describe(redeemFresh.name, () => {
      ...
      it("fails if comptroller tells it to", async () => {
        ...
        expect(
          await redeemFresh(cToken, minter, mintTokens, mintAmount)
        ).toHaveTrollReject("REDEEM_COMPTROLLER_REJECTION");
      });
      ...
      it("fails if exchange calculation fails", async () => {
        if (redeemFresh == redeemFreshTokens) {
          ...
          await expect(
            redeemFresh(cToken, minter, mintTokens, mintAmount)
  ...
  describe("redeem", () => {
    ...
    it("emits a redeem failure if interest accrual fails", async () => {
      ...
      await expect(
        send(cToken, "redeem", [mintTokens], { from: minter })
    ...
    it("returns error from redeemFresh without emitting any extra logs", async () => {
      ...
      await send(masterChef, "harnessSetUserAmount", [0, cToken._address, 1]);
      expect(
        await send(cToken, "redeem", [mintTokens], { from: minter })
      ).toHaveTokenFailure(
        "TOKEN_INSUFFICIENT_CASH",
        "REDEEM_TRANSFER_OUT_NOT_POSSIBLE"
      );
    ...
    it("returns success from redeemFresh and redeems the right amount", async () => {
      ...
      expect(await balanceOf(cToken.underlying, minter)).toEqualNumber(
        mintAmount
      );
    });
    ...
    it("returns success from redeemFresh and redeems the right amount of underlying", async () => {
      expect(
        await send(cToken, "redeemUnderlying", [mintAmount], { from: minter })
      ).toSucceed();
      expect(await balanceOf(cToken.underlying, minter)).toEqualNumber(
        mintAmount
      );
