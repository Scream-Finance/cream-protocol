...
const { makeCToken, preCSLP } = require("../Utils/Compound");
  ...
  describe("_setImplementation", () => {
    describe("ccapable", () => {
      ...
      it("fails due to non admin", async () => {
        ...
        await expect(
          send(
            cToken,
            "_setImplementation",
            [cCapableDelegate._address, true, "0x0"],
            { from: accounts[0] }
          )
        ).rejects.toRevert(
          "revert CErc20Delegator::_setImplementation: Caller must be admin"
        );
      });
      ...
      it("succeeds to have internal cash", async () => {
        ...
        cToken = await saddle.getContractAt("CErc20Delegator", cToken._address);
        expect(
          await send(cToken, "_setImplementation", [
      ...
      it("fails due to non admin", async () => {
        ...
        await expect(
          send(
            cToken,
            "_setImplementation",
            [cslpDelegate._address, true, data],
            { from: accounts[0] }
          )
        ).rejects.toRevert(
          "revert CErc20Delegator::_setImplementation: Caller must be admin"
      ...
      it("fails due to non admin", async () => {
        ...
        await expect(
          send(
            cToken,
            "_setImplementation",
            [cctokenDelegate._address, true, "0x0"],
            { from: accounts[0] }
          )
        ).rejects.toRevert(
          "revert CErc20Delegator::_setImplementation: Caller must be admin"
