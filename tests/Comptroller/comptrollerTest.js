const { address, etherMantissa, both } = require("../Utils/Ethereum");
...
describe("Comptroller", () => {
  ...
  describe("_setLiquidityMining", () => {
    ...
    it("fails if called by non-admin", async () => {
      await expect(
        send(comptroller, "_setLiquidityMining", [liquidityMining._address], {
          from: accounts[0],
        })
      ).rejects.toRevert("revert only admin can set liquidity mining module");
    });
    ...
    it("succeeds and emits a NewLiquidityMining event", async () => {
      const result = await send(comptroller, "_setLiquidityMining", [
        liquidityMining._address,
      ]);
      expect(result).toSucceed();
      ...
      expect(await call(comptroller, "liquidityMining")).toEqual(
        liquidityMining._address
      );
    });
  });
  ...
  describe("_supportMarket", () => {
    ...
    it("fails if not called by admin", async () => {
      ...
      await expect(
        send(cToken.comptroller, "_supportMarket", [cToken._address, version], {
          from: accounts[0],
        })
    ...
    it("fails if asset is not a CToken", async () => {
      ...
      await expect(
        send(comptroller, "_supportMarket", [asset._address, version])
      ).rejects.toRevert();
    });
  ...
  describe("_delistMarket", () => {
    ...
    it("fails if not called by admin", async () => {
      const cToken = await makeCToken(root);
      await expect(
        send(cToken.comptroller, "_delistMarket", [cToken._address], {
          from: accounts[0],
        })
    ...
    it("fails if market not listed", async () => {
      ...
      const asset = await makeToken(root);
      await expect(
        send(comptroller, "_delistMarket", [asset._address])
    ...
    it("fails if market not empty", async () => {
      const cToken = await makeCToken(root);
      expect(
        await send(cToken.comptroller, "_supportMarket", [
          cToken._address,
          version,
        ])
      ).toSucceed();
    ...
    it("succeeds and delists market", async () => {
      ...
      const result = await send(cToken.comptroller, "_delistMarket", [
        cToken._address,
      ]);
    ...
    it("can delist two different markets", async () => {
      ...
      const cToken2 = await makeCToken({ comptroller: cToken1.comptroller });
      expect(
        await send(cToken1.comptroller, "_supportMarket", [
          cToken1._address,
          version,
        ])
      ).toSucceed();
      expect(
        await send(cToken2.comptroller, "_supportMarket", [
          cToken2._address,
          version,
        ])
      ).toSucceed();
      const result1 = await send(cToken1.comptroller, "_delistMarket", [
        cToken1._address,
      ]);
      const result2 = await send(cToken2.comptroller, "_delistMarket", [
        cToken2._address,
      ]);
