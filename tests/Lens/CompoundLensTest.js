...
describe("CompoundLens", () => {
  ...
  describe("cTokenMetadata", () => {
    it("is correct for a cErc20", async () => {
      ...
      await send(cErc20.comptroller, "_setMarketSupplyCaps", [
        [cErc20._address],
        [100],
      ]);
      await send(cErc20.comptroller, "_setMarketBorrowCaps", [
        [cErc20._address],
        [200],
      ]);
      await send(cErc20.comptroller, "_setMintPaused", [cErc20._address, true]);
      await send(cErc20.comptroller, "_setBorrowPaused", [
        cErc20._address,
        true,
      ]);
      expect(
    ...
    it("is correct for a cCollateralCapErc20", async () => {
      let cCollateralCapErc20 = await makeCToken({
        kind: "ccollateralcap",
        supportMarket: true,
      });
      expect(
        cullTuple(
          await call(compoundLens, "cTokenMetadata", [
            cCollateralCapErc20._address,
          ])
        )
      ).toEqual({
    ...
    it("is correct for a cCollateralCapErc20 with collateral cap", async () => {
      ...
      expect(
        await send(cCollateralCapErc20, "_setCollateralCap", [100])
      ).toSucceed();
      expect(
        cullTuple(
          await call(compoundLens, "cTokenMetadata", [
    ...
    it("is correct for a cWrappedNative", async () => {
      let cWrappedNative = await makeCToken({
        kind: "cwrapped",
        supportMarket: true,
      });
      expect(
        cullTuple(
          await call(compoundLens, "cTokenMetadata", [cWrappedNative._address])
        )
      ).toEqual({
  ...
  describe("cTokenMetadataAll", () => {
    ...
    it("fails for mismatch comptroller", async () => {
      ...
      let cCollateralCapErc20 = await makeCToken({
        kind: "ccollateralcap",
        supportMarket: true,
        comptroller: comptroller2,
      }); // different comptroller
      let cWrappedNative = await makeCToken({
        kind: "cwrapped",
        supportMarket: true,
        comptroller: comptroller2,
  ...
  describe("cTokenBalances", () => {
    ...
    it("is correct for cCollateralCapErc20", async () => {
      ...
      await send(cCollateralCapErc20, "harnessSetCollateralBalance", [
        acct,
        mintTokens,
      ]);
      await send(cCollateralCapErc20, "harnessSetCollateralBalanceInit", [
        acct,
      ]);
      let ethBalance = await web3.eth.getBalance(acct);
      expect(
        cullTuple(
          await call(
            compoundLens,
            "cTokenBalances",
            [cCollateralCapErc20._address, acct],
            { gasPrice: "0" }
          )
        )
      ).toEqual({
  ...
  describe("getClaimableSushiRewards", () => {
    ...
    it("gets claimable rewards", async () => {
      ...
      const pendingRewards = await call(
        ...
        [[cToken._address], sushiAddress, minter]
      );
      expect(pendingRewards).toEqualNumber(
        await call(masterChef, "sushiPerBlock", [])
      );
    });
  });
});
