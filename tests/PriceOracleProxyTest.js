...
describe("PriceOracleProxy", () => {
  ...
  beforeEach(async () => {
    ...
    cXSushi = await makeCToken({
      comptroller: cEth.comptroller,
      supportMarket: true,
    });

    crvSwap = await makeCurveSwap({ price: crvLPPrice });
    crvLP = await makeToken({
      kind: "curveToken",
      symbol: "ethCrv",
      name: "Curve pool ethCrv",
      crvOpts: { minter: crvSwap._address },
    }); // ETH base
    crvSwap2 = await makeCurveSwap({ price: crvLPPrice });
    crvLP2 = await makeToken({
      kind: "curveToken",
      symbol: "usdCrv",
      name: "Curve pool usdCrv",
      crvOpts: { minter: crvSwap2._address },
    }); // USD base
    yv1 = await makeToken({ kind: "yvaultToken", yvOpts: { price: yvPrice } });
    yv1CrvLP = await makeToken({
      kind: "yvaultToken",
      yvOpts: { underlying: crvLP, price: yvPrice },
    });
    yv2 = await makeToken({
      kind: "yvaultToken",
      yvOpts: { version: "v2", price: yvPrice },
    });
    yv2CrvLP = await makeToken({
      kind: "yvaultToken",
      yvOpts: { version: "v2", underlying: crvLP, price: yvPrice },
    });
    cCrvLP = await makeCToken({
      comptroller: cEth.comptroller,
      supportMarket: true,
      underlying: crvLP,
    }); // ETH base
    cCrvLP2 = await makeCToken({
      comptroller: cEth.comptroller,
      supportMarket: true,
      underlying: crvLP2,
    }); // USD base
    cYv1 = await makeCToken({
      comptroller: cEth.comptroller,
      supportMarket: true,
      underlying: yv1,
    });
    cYv1CrvLP = await makeCToken({
      comptroller: cEth.comptroller,
      supportMarket: true,
      underlying: yv1CrvLP,
    });
    cYv2 = await makeCToken({
      comptroller: cEth.comptroller,
      supportMarket: true,
      underlying: yv2,
    });
    cYv2CrvLP = await makeCToken({
      comptroller: cEth.comptroller,
      supportMarket: true,
      underlying: yv2CrvLP,
    });
  ...
  describe("getUnderlyingPrice", () => {
    ...
    let setPrice = async (token, price, base) => {
      const mockAggregator = await makeMockAggregator({
        answer: etherMantissa(price),
      });
      await send(oracle, "_setAggregators", [
        [token._address],
    ...
    it("gets price for ETH based cCrvLP", async () => {
      expect(
        await send(oracle, "_setCurveTokens", [
          [crvLP._address],
          [2],
          [0],
          [crvSwap._address],
        ])
      ).toSucceed(); // 2: V3, 0: ETH
    ...
    it("gets price for USD based cCrvLP", async () => {
      ...
      await send(backingOracle, "setDirectPrice", [
        await call(oracle, "usdcAddress"),
        etherMantissa(5, 1e26),
      ]);
      expect(
        await send(oracle, "_setCurveTokens", [
          [crvLP2._address],
    ...
    it("gets price for cYv1", async () => {
      ...
      await send(backingOracle, "setDirectPrice", [
        await call(yv1, "token"),
        etherMantissa(1),
      ]);
      expect(
        await send(oracle, "_setYVaultTokens", [[yv1._address], [0]])
      ).toSucceed(); // 0: v1
    ...
    it("gets price for cYv1 with curve LP underlying", async () => {
      ...
      expect(
        await send(oracle, "_setYVaultTokens", [[yv1CrvLP._address], [0]])
      ).toSucceed(); // 0: v1
      let price = await call(oracle, "getUnderlyingPrice", [
        cYv1CrvLP._address,
      ]);
    ...
    it("gets price for cYv2", async () => {
      // Set underlying price to 1.
      await send(backingOracle, "setDirectPrice", [
        await call(yv2, "token"),
        etherMantissa(1),
      ]);
      expect(
        await send(oracle, "_setYVaultTokens", [[yv2._address], [1]])
      ).toSucceed(); // 1: v2
    ...
    it("gets price for cYv2 with curve LP underlying", async () => {
      ...
      expect(
        await send(oracle, "_setYVaultTokens", [[yv2CrvLP._address], [1]])
      ).toSucceed(); // 1: v2
      let price = await call(oracle, "getUnderlyingPrice", [
        cYv2CrvLP._address,
      ]);
  ...
  describe("_setLPs", () => {
    it("set LPs successfully", async () => {
      expect(
        await send(oracle, "_setLPs", [[cOther._address], [true]])
      ).toSucceed();
    });

    it("fails to set LPs for non-admin", async () => {
      await expect(
        send(oracle, "_setLPs", [[cOther._address], [true]], {
          from: accounts[0],
        })
      ).rejects.toRevert("revert only the admin may set LPs");
    });

    it("fails to set LPs for mismatched data", async () => {
      await expect(
        send(oracle, "_setLPs", [[cOther._address], [true, true]])
    ...
    it("fails to set aggregators for non-admin", async () => {
      await expect(
        send(
          oracle,
          "_setAggregators",
          [[cOther._address], [mockAggregator._address], [0]],
          { from: accounts[0] }
        )
      ).rejects.toRevert(
      ...
      ).rejects.toRevert("revert guardian may only clear the aggregator"); // 0: ETH
    });

    it("fails to set aggregators for mismatched data", async () => {
      await expect(
        send(oracle, "_setAggregators", [[cOther._address], [], [0]])
      ).rejects.toRevert("revert mismatched data"); // 0: ETH
      await expect(
      ...
      await expect(
        send(oracle, "_setAggregators", [[], [mockAggregator._address], [0]])
      ).rejects.toRevert("revert mismatched data"); // 0: ETH
    });
  ...
  describe("_setYVaultTokens", () => {
    ...
    it("fails to set yv tokens for non-admin", async () => {
      await expect(
        send(oracle, "_setYVaultTokens", [[yv1._address], [0]], {
          from: accounts[0],
        })
      ).rejects.toRevert("revert only the admin may set Yvault tokens"); // 0: v1
    });

    it("fails to set yv tokens for mismatched data", async () => {
      await expect(
        send(oracle, "_setYVaultTokens", [[yv1._address], []])
      ).rejects.toRevert("revert mismatched data");
    });

    it("fails to set yv tokens for invalid version", async () => {
      await expect(
        send(oracle, "_setYVaultTokens", [[yv1._address], [2]])
      ).rejects.toRevert("revert");
    });

    it("fails to set yv tokens for mismatched version", async () => {
      await expect(
        send(oracle, "_setYVaultTokens", [[yv1._address], [1]])
      ).rejects.toRevert("revert"); // 1: v2
      await expect(
        send(oracle, "_setYVaultTokens", [[yv2._address], [0]])
      ).rejects.toRevert("revert"); // 0: v1
    });
  });
  ...
  describe("_setCurveTokens", () => {
    ...
    it("fails to set yv tokens for non-admin", async () => {
      await expect(
        send(
          oracle,
          "_setCurveTokens",
          [[crvLP._address], [2], [0], [crvSwap._address]],
          { from: accounts[0] }
        )
      ).rejects.toRevert("revert only the admin may set curve pool tokens"); // 2: V3, 0: ETH
    });

    it("fails to set yv tokens for mismatched data", async () => {
      await expect(
        send(oracle, "_setCurveTokens", [[crvLP._address], [2], [0], []])
      ).rejects.toRevert("revert mismatched data"); // 2: V3, 0: ETH
      await expect(
        send(oracle, "_setCurveTokens", [
          [crvLP._address],
          [2],
          [],
          [crvSwap._address],
      ...
      await expect(
        send(oracle, "_setCurveTokens", [[], [2], [0], [crvSwap._address]])
      ).rejects.toRevert("revert mismatched data"); // 2: V3, 0: ETH
    });
    ...
    it("fails to set yv tokens for incorrect pool", async () => {
      ...
      ).rejects.toRevert("revert incorrect pool"); // 2: V3, 0: ETH
    });
  });
});
