...
async function makeCToken(opts = {}) {
  ...
  switch (kind) {
    ...
    case "ccollateralcap":
      ...
      cToken = await saddle.getContractAt(
        "CCollateralCapErc20DelegateHarness",
        cDelegator._address
      );
    ...
    case "cslp":
      ...
      cDelegator = await deploy("CErc20Delegator", [
        ...
        encodeParameters(
          ...
          [masterChef._address, sushiBar._address, 0]
        ), // pid = 0
      ]);
      cToken = await saddle.getContractAt(
        "CSLPDelegateHarness",
...
async function makeToken(opts = {}) {
  ...
  if (kind == "erc20") {
    ...
    const cToken = await deploy("CTokenHarness", [
      quantity,
      name,
      decimals,
      symbol,
